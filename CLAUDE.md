...
- FLO's `serve` subcommand (`BDS-FAL-DAEMON-v0.1`) is its only HTTP-serving surface: one read-only route, `GET /api/v1/capabilities/{capability_id}`, default-off unless `FA_LOCAL_SERVE_ENABLED` is set, port 8012 by default. It is not a general HTTP service -- no write-capable route, no execution-over-HTTP route, and `route`/`execute` stay CLI-only. Everything else FA Local does over HTTP remains client-only (`integrations/df_local`, `integrations/neuronforge_local`).
