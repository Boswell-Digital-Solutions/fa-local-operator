...
- broad cross-service adapter integrations beyond the Cortex Gnat proving slice above (which itself still has no forensic recording and no deadline/timeout enforcement on the dispatch subprocess call); NeuronForge-Local integration is unstarted. DF-Local's execution-bridge writeback (above) is delivered; other DF-Local integration surfaces remain unstarted.
...
Still not delivered: forensic recording for Gnat dispatch runs (`ForensicRecordKind` is built entirely around FA Local's own `RouteDecision`/`ExecutionStatus` domain, which a Cortex-initiated Gnat run has no equivalent of — needs its own forensic-event contract extension, not a bolt-on to this pipeline), and deadline/timeout enforcement on the subprocess call (a disclosed gap — the call blocks until Cortex's CLI exits on its own).
...
- broad cross-service adapter integrations (adapters reaching real peer services, not local-only delivery) — the Cortex Gnat proving slice is delivered and live-verified end to end (negotiate → dispatch, via `fa-local-run gnat-dispatch`, including the negotiation-to-dispatch bridge from `GnatDispatchShard`), but still has no forensic recording and no deadline/timeout enforcement on the dispatch subprocess call, and NeuronForge-Local/DF-Local integrations are unstarted — see `doc/system/10_service-contract/02-contract-surface.md`
