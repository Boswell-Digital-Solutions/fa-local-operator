...
- broad cross-service adapter integrations beyond the Cortex Gnat proving slice above (which itself still has no forensic recording and no negotiation-to-dispatch bridge from `GnatDispatchShard`); NeuronForge-Local integration is unstarted. DF-Local's execution-bridge writeback (below) is delivered; other DF-Local integration surfaces remain unstarted.
...
Bounded to the two worker types `DECISIONS/0018` (COR) authorizes for this proving slice (`markdown_syntax`, `plain_text_syntax`, via `AUTHORIZED_WORKER_TYPES`) — every other worker type the registry otherwise supports is refused before ever spawning a process, not left for Cortex's own CLI to reject. `GnatShardDispatchRequest` carries the full `GnatShard.v1` contract fields plus the real `local_path` that contract deliberately excludes (COR's `source_path_token` is a one-way derived diagnostic, not a reversible path reference); this adapter takes that request as caller-supplied, already-complete input, and does *not* derive it from `GnatDispatchShard` (the negotiation-time envelope's shard summary, which lacks `source_path_token`, `media_type`, `max_bytes`, and `local_path` entirely) — bridging admission negotiation into a full runnable shard descriptor is a separate, undelivered concern.

`GnatDispatchPipelineService` (`src/app/gnat_dispatch_pipeline_service.rs`) closes the loop `negotiate` alone never did: negotiate, and only when the negotiated posture is `ReadyForFaLocalDispatch`, deliver every declared shard through a `GnatShardDeliveryAdapter` and collect its real outcome. A denied run dispatches nothing; a `SerialFallbackPermitted` run also dispatches nothing here, since running the shards itself in that case is Cortex's own job (`DECISIONS/0019`), not this pipeline's. Before negotiating at all, it checks that the caller-supplied `GnatShardDispatchRequest` for every declared shard agrees with the envelope's own `run_id`/`shard_id`/`worker_type`/`source_ref` — a consistency check, not a derivation; a mismatch is refused, never silently reconciled. Exposed as `fa-local-run gnat-dispatch --envelope <FILE> --shards <FILE> --cortex-repo-root <DIR> [--cortex-python <BINARY>]`, exit 0 only when the run was dispatched and every shard's receipt reports `state: "complete"`. Live-verified end to end against the real COR checkout: a two-shard envelope (`markdown_syntax` + `plain_text_syntax`) negotiated, admitted, both shards dispatched to two real Cortex workers, both receipts `complete`.

Still not delivered: forensic recording for Gnat dispatch runs (`ForensicRecordKind` is built entirely around FA Local's own `RouteDecision`/`ExecutionStatus` domain, which a Cortex-initiated Gnat run has no equivalent of — needs its own forensic-event contract extension, not a bolt-on to this pipeline), the negotiation-to-dispatch bridge above, and deadline/timeout enforcement on the subprocess call (a disclosed gap — the call blocks until Cortex's CLI exits on its own).
...
- broad cross-service adapter integrations (adapters reaching real peer services, not local-only delivery) — the Cortex Gnat proving slice is delivered and live-verified end to end (negotiate → dispatch, via `fa-local-run gnat-dispatch`), but still has no forensic recording and no negotiation-to-dispatch bridge from `GnatDispatchShard`, and NeuronForge-Local/DF-Local integrations are unstarted — see `doc/system/10_service-contract/02-contract-surface.md`
