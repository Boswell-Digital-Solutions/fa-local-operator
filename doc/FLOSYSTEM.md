...
| Part | File | Contents |
| --- | --- | --- |
| §1 | `00_overview/01-overview-charter.md` | Identity, charter, FA-Local execution-worker role |
...
| §4 | `30_dependencies/04-dependencies.md` | Local-plane peers + contract dependencies |
...
- `fa-local-run serve` (`BDS-FAL-DAEMON-v0.1`, `docs/plans/active/BDS_FAL_DAEMON_v0.1/`): a read-only daemon mode with one route, `GET /api/v1/capabilities/{capability_id}`. The route returns the matching `CapabilityRecord` from the capability registry that `--registry-file` names. The mode is default-off: without `FA_LOCAL_SERVE_ENABLED`, `serve` exits and binds no listener. Each request must carry an unexpired Ed25519 bearer JWS with scope `capability:read`, signed by a configured key. Forge_Command's token authority mints these tokens. The keys come from `FA_LOCAL_SERVE_PUBLIC_KEYS` (`--public-keys-env` changes the variable name). An empty or unset public-key map denies every request. The blocking `tiny_http` listener binds `127.0.0.1` on port 8012 by default (`--port` overrides); forge `PORT_REGISTRY.md` registers 8012 for it. `SIGHUP` reloads the registry file; a failed reload keeps the previous good registry. The process holds the loaded registry in memory and writes nothing to disk. No list, write, or execution route exists; `route` and `execute` stay CLI-only.

What is still intentionally not delivered:

- broad cross-service adapter integrations beyond the Cortex Gnat proving slice and the first NeuronForge-Local proving slice above (which admits only the one task `ADR-002` names -- investigated, not just unstarted: `BDS-FAL-NFL-ADMISSION-v0.1` CP0/WP00 found two technically-admissible candidates with no demonstrated FA-Local-side consumer and closed GATE-00 with "no next task admission justified," `KI-FLO-20260918-007`). DF-Local's execution-bridge writeback (above) is delivered; other DF-Local integration surfaces remain unstarted.
- broader daemon or API surfaces: a list route, a write-capable route, or an execution-over-HTTP route. `BDS-FAL-DAEMON-v0.1` excludes them, and each needs its own authorization.
- persistence layer beyond forensic evidence (e.g. durable policy/capability/execution state across restarts) -- not independently actionable, because no FA Local process loses state across a restart. Every subcommand except `serve` runs once and exits. `serve` holds only the loaded registry and public-key map in memory, and a restart re-reads both. `BDS-FAL-DAEMON-v0.1` ruled that registry in-memory and file-backed, with no durable store (OD-1). `forge-local-runtime`'s accepted boundary doctrine assigns any future durable state to DF Local Foundation, not FA Local (`KI-FLO-20260918-006`). A persistence proposal becomes concrete only when a real state-loss gap appears.
...
The Nmap preflight adapter is bounded to a declared `local_process_spawn` capability and execution plan, does not run scans, does not accept free-form arguments, and does not create a networked daemon surface. Missing `nmap` runtime truth can be represented as a degraded execution status and recorded through the existing minimized forensic-event path. The review-package emitter remains intentionally bounded to the two current review postures only and does not introduce generic workflow behavior beyond `review_required` and `explicit_operator_approval`. Still not delivered: a daemon/API surface beyond the one read-only `serve` lookup route (§1), and persistence beyond forensic evidence.
...
Exposed as `fa-local-run neuronforge-dispatch --scene <FILE> [--neuronforge-url <URL>] [--model <ID>] [--forensic-export <FILE>|--forensic-sqlite <FILE>]`, exit 0 only on `Completed`. Live-verified end to end against a real running `neuronforge-local-operator` service and a real local Ollama model: a real scene POSTed through the real CLI produced a genuine structured style-analysis candidate (`schema_validation_status: "valid"`, every `registry_guardrails` flag `false`, exit 0); with the service stopped, the same CLI call correctly reported `dispatch_unavailable` with a truthful connection-refused summary (exit 1).

Every run is also recorded as forensic truth via `integrations::neuronforge_local::NeuronForgeTaskDispatchForensicEvent` (`schemas/neuronforge-task-dispatch-forensic-event.schema.json`) — a separate contract from `forensic-event.schema.json`, the same reason Gnat dispatch's own forensic event is separate. `NeuronForgeDispatchPipelineService::run` composes `HttpNeuronForgeLocalAdapter::dispatch_task` with forensic recording, taking an optional `forensic_export_adapter: Option<&dyn NeuronForgeForensicEventExportAdapter>`. Unlike Gnat dispatch (one negotiation event plus one per shard), one NeuronForge-Local dispatch run has no separate negotiation phase and records exactly one event: `outcome` (`completed`/`not_completed`/`dispatch_unavailable`) and `receipt_validation_status` (`valid`/`degraded`/`failed` — structurally required for `completed`/`not_completed`, structurally forbidden for `dispatch_unavailable`) mirror the same truthful-outcome shape `GnatDispatchForensicEvent` uses for `shard_outcome`/`receipt_state`. `JsonlNeuronForgeForensicExportAdapter`/`SqliteNeuronForgeForensicStore` (`src/integrations/neuronforge_local/forensic_export.rs`) mirror Gnat dispatch's own export sinks; a failed export fails the run closed, reusing `ForensicService::map_export_result` rather than duplicating its validation. Exposed via the same `--forensic-export`/`--forensic-sqlite` flags on `neuronforge-dispatch` above.

Live-testing this found and fixed a real bug (`KI-FLO-20260918-005`, `docs/KNOWN_ISSUES.md`): both `GnatDispatchForensicEvent::validate()` and the new `NeuronForgeTaskDispatchForensicEvent::validate()` checked `summary.len()` (byte length) against a string each pipeline's own `bounded_summary()` helper had already truncated to exactly 160 *characters* (matching the JSON Schema's own Unicode-codepoint-based `maxLength: 160`) — a real, long `ureq` connection-refused message reproduced this live: the truncated string was 160 codepoints but 162 bytes (the trailing `'…'` alone is 3 UTF-8 bytes), so validation rejected the pipeline's own output and the whole run failed closed instead of recording a truthful `dispatch_unavailable` outcome. Fixed in both files by checking `summary.chars().count()` instead. Live-verified end to end, both outcomes, against the real running service: a real completed dispatch and a real `dispatch_unavailable` (service stopped, connection-refused message truncated correctly) both recorded and exported via `--forensic-sqlite`, confirmed by querying the SQLite store directly.

## External service-status projection (FC-LTA-P007)

FA Local exposes this projection through `domain::service_status::build_canonical_service_status_envelope()`, reached from the `fa-local-run canonical-status` CLI subcommand. Forge_Command reads it the same way it already reads Cortex's projection: as a CLI subprocess. It runs the compiled `fa-local-run` binary with the `canonical-status` argument and parses the one line of JSON on stdout. FA Local adds no HTTP route for this projection; `canonical-status` stays CLI-only. FA Local's one HTTP-serving route is the read-only `serve` capability lookup (§1).
